...
| ⚠️ **실사용자 말투로 물으면** | **R@1 0.62 · R@5 0.88** (제품 선택, 홀드아웃 320) | 같은 정답, 표현만 구어체 (§10) + 그 기준으로 재튜닝 (§11) |
...
### 11. 정직한 지표를 1급으로 승격하고, 그 기준으로 재튜닝했다

§10에서 편향을 확인한 뒤 당연한 다음 질문: **지금의 튜닝(BM25 가중치·융합 구성)도 편향된
평가셋에 맞춰진 것 아닌가?** 실사용자 재작성을 640문항 전체로 확장하고
(어휘 겹침 46.5%→22.8%, 640문항 중 608건 감소 확인), 그 기준으로 융합을 다시 골랐습니다.

**과적합 방지 프로토콜** — 640으로 튜닝하고 640으로 보고하면 이번엔 *선택 편향*을 만듭니다.
그래서 **qid 짝수 320으로 구성을 고르고, 홀수 320에서만 유의성을 확정**했습니다 (`tune_report.py`).

| 라이브 경로 | BM25 가중치 스윕 {1.0, 0.5, 0.25, 0} | 홀드아웃(320) 판정 |
|---|---|---|
| 제품 선택 (매뉴얼 스코프) | **0 (dense 후보만 → 리랭커)** 승 | **채택** — R@1 +3.1pp CI[+0.009,+0.056] · R@5 +6.9pp · McNemar p=0.013 |
| 카테고리 스코프 | 0 근소 우위 | 중립 (전 지표 미유의) — 손해 없음 |
| 제품 미선택 (글로벌) | 0.5 근소 우위 | **기각** — 미유의 (p=1.0), **기존 융합 유지** |

**두 경로가 반대 결론인 이유가 §10의 메커니즘과 정확히 맞물립니다.** 스코프가 좁으면(한 매뉴얼
~70청크) dense가 이미 정답 근처를 찾고, 실사용자 말투에서 붕괴한 BM25는 리랭커에 들어갈
후보 10자리를 노이즈로 채웁니다 → 빼는 게 유의한 이득. 글로벌(6,426청크)에선 BM25가
"dense가 놓친 다른 종류의 후보"를 공급하는 다양성 가치가 남아 있어 제거가 오히려 최하위였습니다.

→ 라이브 반영: **스코핑 시 BM25 생략**(dense→리랭커), 글로벌은 융합 유지. 부수로 스코프 경로의
BM25 호출이 사라져 지연도 소폭 줄었습니다. 실사용자 기준 최종(홀드아웃): 제품 선택 시
**R@1 0.622 · R@5 0.878**.

*(정직 각주: §10 파일럿 200문항과 이 절의 640문항은 재작성 주체가 달라 — 전자가 더 공격적인
구어체(겹침 18.2%) — 절대치가 다릅니다. 여기 스윕은 전 구성이 같은 640 평가셋을 쓰므로
비교 자체는 공정합니다.)*
...
**정직한 한계**
- **원본 평가셋은 낙관 편향돼 있고, 그 크기를 측정했습니다(§10): 실사용자 말투에서 R@1 −32pp.**
  문서 유래 640문항 표는 검색기 간 *상대 비교*로는 유효하지만 절대 성능으로 읽으면 안 됩니다.
  실사용자 평가셋(640)도 LLM 재작성이라 한계가 있고, **사람이 처음부터 쓴 질문은 아직 없습니다**
...
- [x] 실사용자 평가셋 640으로 확대 + 그 기준으로 재튜닝 — 스코핑 시 BM25 제거 채택(홀드아웃 유의),
      글로벌은 융합 유지(§11)
